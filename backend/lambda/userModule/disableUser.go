...
func disableUser(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	disableUser := &cognito.AdminDisableUserInput{
		UserPoolId: aws.String(cognitoClient.UserPoolID),
		Username:   aws.String(credentials.Username),
	}

	response, err := cognitoClient.CognitoClient.AdminDisableUser(disableUser)
	if err != nil {
